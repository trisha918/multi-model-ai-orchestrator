  ...
  async function once(attempt) {
    ...
    return new Promise(resolve => {
      ...
      child.on('close', (code, signal) => {
        ...
        resolve({
          command: label,
          // Windows can report 0 after taskkill closes a child. A deadline is
          // still a failed invocation and must never look like success.
          exitCode: outputLimit || (timedOut && code === 0) ? 1 : code,
          outputLimit,
          signal,
          timedOut,
          durationMs: Date.now() - started,
