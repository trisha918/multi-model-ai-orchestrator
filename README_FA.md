...
**برای شروع، [راهنمای ساده و کامل فارسی](docs/START-HERE-FA.md) را بخوانید.** نصب، دستورهای مستقیم، استفاده در Cursor، دریافت خروجی، تاریخچه، بودجه، حافظه، صف و اتومیشن جداگانه توضیح داده شده‌اند. برای استفادهٔ محلی لازم نیست اتومیشن GitHub را فعال کنید.

امکانات جدید روی شاخهٔ توسعهٔ `codex/reliable-local-and-optional-automation` هستند و هنوز انتشار رسمی نشده‌اند. دستور نصب همین شاخه در راهنمای فارسی آمده است. [راهنمای انگلیسی](docs/START-HERE.md) و [قراردادهای فنی و محدودیت‌ها](docs/LOCAL-MODULES.md) نیز موجودند. دستورهای نسخهٔ قبلی در ادامه همچنان پشتیبانی می‌شوند.
